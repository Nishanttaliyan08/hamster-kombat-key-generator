...
// const shareText =
//   "Unlock your #HamsterKombat game keys for free with our reliable Key Generator! ðŸ¹ðŸ’¥ðŸŽ®ðŸ”‘\n\nCheck it out now: https://evmlord.dev/hamster-codes/\n\nFeel free to repost ðŸ”¥";

let currentAppConfig = gamePromoConfigs.ChainCube2048;
var currentLanguage = defaultLanguage;
var keygenActive = false;

const shareUrl = "https://whatsapp.com/channel/0029Va9vJ3u3rZZjFirRxj0u";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const sharePgLink = document.getElementById("sharePgLink");
  ...
  shareTgBtn.addEventListener("click", async () => {
    const url = encodeURIComponent("https://evmlord.dev/hamster-codes/");
    const text = encodeURIComponent(await getTranslation("shareText"));
  ...
  shareXBtn.addEventListener("click", async () => {
    const url = encodeURIComponent("https://evmlord.dev/hamster-codes/");
    const text = encodeURIComponent(await getTranslation("shareText"));
    const twitterUrl = `https://x.com/intent/post?url=${url}&text=${text}`;
  ...
  sharePgLink.addEventListener("click", async (event) => {
    navigator.clipboard
      .writeText(await getTranslation("shareText"))
      .then(async () => {
        event.target.innerText = await getTranslation("sharePgLinkCopied");
        event.target.style.backgroundColor = "#28a745";
        setTimeout(async () => {
          event.target.innerText = await getTranslation("sharePgLinkBtn");
          event.target.style.backgroundColor = "#f5f5f5";
        }, 3000);
      });
  });

  creatorChannelBtn.addEventListener("click", async () => {
...
function applyTranslations(translations) {
  ...
  document.getElementById("keyCountLabel").innerText = keygenActive
    ? translations.selectKeyCountLabel_selected +
      document.getElementById("keyCountSelect").value
    : translations.selectKeyCountLabel;
  document.getElementById("startBtn").innerText = translations.generateButton;
  document.getElementById("generatedKeysTitle").innerText =
    translations.generatedKeysTitle;

  document.getElementById("shareXBtn").innerText = translations.twitterButton;
  document.getElementById("shareWABtn").innerText = translations.whatsAppButton;
  document.getElementById("shareTgBtn").innerText = translations.telegramButton;
  document.getElementById("shareTitle").innerText = translations.shareTitle;
  document.getElementById("copyAllBtn").innerText =
    translations.copyAllKeysButton;
  document.getElementById("sharePgLink").innerText =
    translations.sharePgLinkBtn;
  document.getElementById("gameSelectLabel").innerText =
    translations.selectGameLabel;

  document.querySelectorAll(".copyKeyBtn").forEach((button) => {
    button.innerText = translations.copyKeyButton || "Copy Key";
  });
  document.getElementById("creatorChannelBtn").innerText =
...
document.getElementById("startBtn").addEventListener("click", async () => {
  ...
  keyCountLabel.innerText =
    (await getTranslation("selectKeyCountLabel_selected")) + keyCount;
  ...
  if (keys.length > 1) {
    ...
    keysList.innerHTML = `
            <div class="key-item">
                ...
                <button class="copyKeyBtn copy-button" data-key="${
                  keys[0]
                }">${await getTranslation("copyKeyButton")}</button>
            </div>
        `;
  }
